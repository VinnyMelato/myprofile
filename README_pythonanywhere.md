...
Notes
- This project currently uses sqlite by default. If you prefer MySQL/Postgres on PythonAnywhere, update `DATABASES` in `myprofile/settings.py` or set a `DATABASE_URL` and use dj-database-url.
- The repo already reads SECRET_KEY and DEBUG from the environment via python-decouple.
